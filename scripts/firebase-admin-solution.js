...
async function createTestData() {
  try {
    // Create admin user with timestamp to avoid duplicates
    ...
    // Create parent user with timestamp to avoid duplicates
    const parentId = await createUser(
      `parent-${timestamp}@example.com`,
      'Password123!',
      'Parent User',
      'parent'
    );
...
// Run test data creation if script is executed directly
