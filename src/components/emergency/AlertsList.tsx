...
export default function AlertsList({ role, limit }: AlertsListProps) {
  const [alerts, setAlerts] = useState<(MissingChildAlert & { child?: ChildProfile })[]>([]);
  const [loading, setLoading] = useState(true);
  
  const { user } = useAuth();
  const { queryDocuments, getDocument, where, orderBy } = useFirestore();
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      ...
      try {
        ...
        // Different query constraints based on role
        let constraints = [];
        
        if (role === 'parent') {
          // Parents only see alerts for their own children
          constraints = [where('reporterId', '==', user.uid), orderBy('dateReported', 'desc')];
        } else if (role === 'school') {
          // Schools see alerts for children in their school
          // This would require a more complex query in a real system
          constraints = [orderBy('dateReported', 'desc')];
        } else {
        ...
        const alertsData = await queryDocuments<MissingChildAlert>('alerts', constraints);
    ...
    fetchAlerts();
  }, [user, role, limit, queryDocuments, getDocument, where, orderBy]);
